...
namespace Emma
{
	...
	void EmmaWindow::Init(const WindowProps &props)
	{
		...
		CORE_ASSERT(Window, "Window Creation Failed!", SDL_GetError());
		...
		CORE_ASSERT(GPUDevice, "GPU Device Creation Failed!", SDL_GetError());
		CORE_ASSERT_FUNC(SDL_ClaimWindowForGPUDevice(GPUDevice, Window), "Failed to claim window for GPU Device!", SDL_GetError());
